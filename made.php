...
$string = 'ffprobe -v error -show_entries format=duration -of default=noprint_wrappers=1:nokey=1 '.$file;
$avg = exec($string);
$avg = round($avg);
for($i=300;$i<=$avg-500;$i+=mt_rand(60,300))
{
	...
	$len = mt_rand(60,300);
	$cmd = 'ffmpeg -y -i  '.$file.' -ss '.$start.' -t '.$len.' -vcodec copy -acodec copy '.$i.'.mp4';
	echo $cmd;
	exec($cmd);//exit;
